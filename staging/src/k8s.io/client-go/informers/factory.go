...
type sharedInformerFactory struct {
	...
	// zhou: a custom resync period for the specified informer types.
	customResync map[reflect.Type]time.Duration
	transform    cache.TransformFunc
	// zhou: how many types need to be handled by informer ???
...
// zhou: similar with NewSharedInformerFactory(), could specify the namespace.
...
func NewSharedInformerFactoryWithOptions(client kubernetes.Interface, defaultResync time.Duration, options ...SharedInformerOption) SharedInformerFactory {
	...
	// zhou: options could be generated by "WithNamespace()"/...
	//       Used to fill fields of "sharedInformerFactory{}"
	// Apply all options
...
func (f *sharedInformerFactory) Start(stopCh <-chan struct{}) {
	...
	for informerType, informer := range f.informers {
		if !f.startedInformers[informerType] {
			...
			// zhou: each gvr, will run in a separate goroutine.
			go func() {
				defer f.wg.Done()
				informer.Run(stopCh)
			}()
...
// zhou: just like "kubernetes.Interface" (implemented by "kubernetes.ClientSet"), by
//       "informers.SharedInformerFactory" (implemented by "informers.sharedInformerFactory"),
//       we could get "cache.SharedIndexInformer" for each type.
